...
def NCMPDataIter_graph(sample_dir_path, idx, test_idx, feature_dimension, time_length, win_len=50, win_stride=1,
                       sampling=10, sub=10):
...
def minmaxNor():
    ...
    for test_idx in range(4):
        train_sample_array, train_label_array, test_sample_array, test_label_array, max_RUL = \
            NCMPDataIter_graph(sample_dir_path, idx=1, test_idx=test_idx, feature_dimension=5, time_length=10,
                               win_len=50)
...
def save_ele_adj_testing(samples, datasub, time_length=50):
    ...
    for i in range(train_samples.shape[0]):
        ...
        train_adj_lst.append(one_sample_adj_lst)
    train_adjs = np.asarray(train_adj_lst)
    np.savez('./N-CMAPSS/Samples_whole/test_adjs_{}_w{}.npz'.format(str(datasub), str(time_length)),
             test_adjs=train_adjs)
...
if __name__ == '__main__':
    ...
    for test_idx in range(1):
        train_sample_array, train_label_array, test_sample_array, test_label_array, max_RUL = \
            NCMPDataIter_graph(sample_dir_path, idx=1, test_idx=test_idx, feature_dimension=1, time_length=50,
                               win_len=50)
