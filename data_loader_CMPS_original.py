...
class CMPDataIter(data.IterableDataset):
    ...
    def __init__(self, data_root, data_set, max_rul, seq_len):
        ...
        self.train_x, self.train_ops, self.train_y, \
            self.test_x, self.test_ops, self.test_y, \
            self.train_normalized, self.test_normalized \
            = self._process(self.train_data_df, self.test_data_df, self.test_truth)
...
#         self.train_x, self.train_ops, self.train_handcraft, self.train_y, \
#             self.test_x, self.test_ops, self.test_handcraft, self.test_y = \
#                 self._process(self.train_data_df, self.test_data_df, self.test_truth)
        
#         train_adjs = \
#             np.load(data_root + 'CMAPSSData/train_samples/train_adjs_FD00{}.npz'.format(data_set[-1]))['train_adjs']
#         self.train_adjs = train_adjs#.tolist()
#         test_adjs = \
#             np.load(data_root + 'CMAPSSData/test_samples/test_adjs_FD00{}.npz'.format(data_set[-1]))['test_adjs']
#         self.test_adjs = test_adjs#.tolist()
...
#         out_y = self.out_y[self.start: self.end]
#         out_adjs = self.out_adjs[self.start: self.end]
#         out_ops = out_adjs
...
if __name__ == '__main__':
    test = CMPDataIter('D://PhD_in_Sg/research related/datasets/RUL', data_set='FD001', max_rul=200, seq_len=45, net_name=1)
    data_visualization = test.train_x
    # print(np.shape(data_visualization))

    import matplotlib.pyplot as plt

    # color = ['aliceblue','beige','cadetblue', 'darkblue','dodgerblue','dodgerblue','green','lightblue']
    # for i in range(8):
    #
    #     plt.plot(data_visualization[:,i], color = color[i])
    #     plt.savefig('./sensor{}.jpg'.format(i))
    #     # plt.plot(data_visualization[:,1])
    #
    #     plt.show()
